...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(1, 0)
        x = torch.einsum('ij,k->ikj', x, torch.ones(self.out_dim, ).to(self.device)).reshape(batch_size,
        ...
        y = torch.sum(y.reshape(batch_size, self.out_dim, self.in_dim), dim=2)  # shape (batch, out_dim)
